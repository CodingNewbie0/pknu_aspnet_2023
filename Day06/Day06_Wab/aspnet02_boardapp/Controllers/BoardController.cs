...
{
    // https://localhost:7183/Board/Index
    public class BoardController : Controller
    {
        ...
        public IActionResult Index(int page = 1) // 게시판 최초화면 리스트
        {
            ...
            if (totalcount % pageSize > 0) { totalPage++; } // 나머지글이 있으면 전체페이지를 1증가(2)
        ...
        // https://localhost:7183/Board/Create
        ...
        // https://localhost:7183/Board/Edit
        [HttpGet]
        ...
        public IActionResult Edit(Board board)
        {
            try
            {
                ...
                TempData["succeed"] = "새 게시글을 수정하였습니다"; // 성공메세지
            }
            catch (Exception)
            {
                TempData["error"] = "비정상적인 오류가 발생하였습니다."; // 실패메세지
            }

            return RedirectToAction("Index", "Board");
        }

        // https://localhost:7183/Board/Delete
        [HttpGet]
        public IActionResult Delete(int? Id)
        {
            // HttoGet Edit Action의 로직과 동일
            if (Id == null || Id == 0)
            {
                return NotFound(); // Error.cshtml이 표시
            }
        ...
        public IActionResult DeletePost(int? Id)
        {
            ...
            TempData["succeed"] = "게시글이 정상적으로 삭제되었습니다"; // 성공메세지
